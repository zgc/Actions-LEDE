...
if [ "${SMARTDNS_ENABLED}" = "1" ]; then
  while [ $tries -lt 5 ]; do
    ...
    NSLOOKUP=$(nslookup -port="${SMARTDNS_PORT}" "$ALIYUN" 127.0.0.1 2>/dev/null | grep -c 'Name:')
    if [ "${NSLOOKUP}" -ne 0 ]; then
      echo "$DATE check smartdns connect: OK" >>$LOG
      exit 0
    else
      echo "$DATE check $DNSPOD start" >>$LOG
      NSLOOKUP=$(nslookup -port="${SMARTDNS_PORT}" "$DNSPOD" 127.0.0.1 2>/dev/null | grep -c 'Name:')
      if [ "${NSLOOKUP}" -ne 0 ]; then
        echo "$DATE check smartdns connect: OK" >>$LOG
        exit 0
      else
  ...
  echo "$DATE smartdns disable, skip" >>$LOG
fi
